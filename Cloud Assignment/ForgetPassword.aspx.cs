...
namespace Cloud_Assignment
{
    public partial class ForgetPassword : System.Web.UI.Page
    {
        ...
        public class Type {
            public static string strSelect, idType;
        }
        ...
        protected void btnSend_Click(object sender, EventArgs e)
        {
            ...
            {
                Type.strSelect = "SELECT * FROM [Customer] WHERE Email = @Email";
                Type.idType = "CustomerID";
            }
            else
            {
                Type.strSelect = "SELECT * FROM [Staff] WHERE Email = @Email";
                Type.idType = "StaffID";
            }
            

            SqlCommand cmdSelect = new SqlCommand(Type.strSelect, con);
                    ...
                    if (txtEmail.Text.Equals(dtr["Email"]))
                    {
                        ...
                        using (MailMessage mail = new MailMessage())
                        {
                            ...
                            mail.Body = "Dear " + dtr["Username"].ToString() + ",<br /><br /> Please click the link below to reset your password: <br />"
                                + HttpContext.Current.Request.Url.AbsoluteUri.Replace("?view=0", "?view=1") + "&id=" + dtr[Type.idType].ToString() + "&time=" + DateTime.Now.ToString("s") + "  <br/>This link only available for 15 minutes. <br /> Thank you! <br /><br /> Best Regards, <br /> Smart Personal Scheduler.";
        ...
        protected void btnConfirm_Click(object sender, EventArgs e)
        {
            if (TextBox2.Text.Equals(TextBox3.Text))
            {
                ...
                if (Type.idType== "CustomerID") {
                    strUpdate = "UPDATE [Customer] SET Password = @Password WHERE CustomerID = @UserID";
                }
                else
                {
                    strUpdate = "UPDATE [Staff] SET Password = @Password WHERE StaffID = @UserID";
                }
